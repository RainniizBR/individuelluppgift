...
// Create areas for total amount, lists of selected items and rating filter input
const cartArea = document.getElementById("cartArea");
const totalAmountArea = document.createElement("p");
const headerShoppingCartArea = document.createElement("p");
const shoppingCartArea = document.createElement("p");
const ratingArea = document.createElement("p");
let totalAmount = 0;

cartArea.appendChild(ratingArea);
cartArea.appendChild(totalAmountArea);
cartArea.appendChild(headerShoppingCartArea);
cartArea.appendChild(shoppingCartArea);

showProductItems(shopItems); // Function for showing data from array shopItems

// Function for showing total amount and lists of selected items after clicking on the images
function imgClick(image, price, name) {
  image.style.border = "5px solid pink";
  totalAmount += parseInt(price);
  totalAmountArea.innerHTML = `Total: ${totalAmount}`;
  headerShoppingCartArea.innerHTML = `<h3><strong>Shopping Cart</strong></h3>`;
  shoppingCartArea.innerHTML =
    shoppingCartArea.innerHTML + `${name} - ${price}<br>`;
}

// Function for filtering items from array
function ratingClick() {
  let ratingItems = shopItems.filter(function (item) {
    return item.rating == document.getElementById("rating").value;
  });
  showProductItems(ratingItems); // Show data from filtered array
}

// Function for showing all items information
function showProductItems(productData) {
  const productArea = document.getElementById("root");
  productArea.innerHTML = ""; // Clear the whole item area when it shows the filtered data

  productData.forEach((shopItem) => {
    ...
    image.src = shopItem.images[0].src.small;
    image.alt = shopItem.images[0].alt;
    image.setAttribute(
      "onclick",
      `imgClick(this, '${shopItem.price}', '${shopItem.name}')`
    );

    const description = document.createElement("p");
    description.innerHTML = `<strong>Product's Description:</strong> "${shopItem.description}"`;

    const price = document.createElement("p");
    price.innerHTML = `<strong>Price:</strong> ${shopItem.price}`;

    const rating = document.createElement("p");
    rating.innerHTML = `<strong>Rating:</strong> ${shopItem.rating}`;

    const stock = document.createElement("p");
    stock.innerHTML = `<strong>Stock:</strong> ${shopItem.stock}`;
    ...
    productArea.appendChild(wrapper);
  });
}
